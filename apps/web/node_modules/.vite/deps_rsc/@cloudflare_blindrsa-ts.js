import { t as require_builtin_esm_external_require_crypto } from "./builtin_esm-external-require-crypto-DsuVPwdG.js";
//#region ../../node_modules/@cloudflare/blindrsa-ts/lib/src/sjcl/index.js
...
(function() {
	function getCryptoModule() {
		try {
			return require_builtin_esm_external_require_crypto();
		} catch (e) {
			return null;
		}
	}
...
sjcl.json = {
	...
	*/
	_filter: function(src, filter) {
		var out = {}, i;
		for (i = 0; i < filter.length; i++) if (src[filter[i]] !== void 0) out[filter[i]] = src[filter[i]];
		return out;
	}
};
...
//#endregion
//#region ../../node_modules/@cloudflare/blindrsa-ts/lib/src/util.js
...
function inverseMod(x, p) {
	if (!(p.getLimb(0) & 1)) {
		if (!(x.getLimb(0) & 1)) throw new Error("inverseMod: The given number is not invertible.");
		...
		if (old_r > 1n) throw new Error("inverseMod: The given number is not invertible.");
		if (old_s < 0n) old_s += BigInt(p.toString());
...
function prepare_sjcl_random_generator() {
	...
	while (!sjcl.random.isReady(void 0)) sjcl.random.addEntropy(Array.from(crypto.getRandomValues(new Uint32Array(4))), 128, source);
}
//#endregion
//#region ../../node_modules/@cloudflare/blindrsa-ts/lib/src/blindrsa.js
var PrepareType;
...
function millerRabinTest(n, SEC_PARAM = 20) {
	...
	for (let i = 0; i < SEC_PARAM; i++) {
		let y = sjcl.bn.random(nMinusOne, SJCL_PARANOIA).powermod(r, n);
		if (!y.equals(1) && !y.equals(nMinusOne)) {
			...
			if (!y.equals(nMinusOne)) return false;
		}
	}
	return true;
}
function generatePrime(bitLength, NUM_TRIES_PRIMALITY = 20) {
	...
	for (let i = 0; i < bitLength; i++) twoToN.doubleM();
	twoToN.normalize();
	let prime;
	let i = 0;
	do {
		prime = sjcl.bn.random(twoToN, SJCL_PARANOIA);
		if ((prime.getLimb(0) & 1) == 0) prime = prime.addM(1).normalize();
		i++;
	} while (!millerRabinTest(prime, NUM_TRIES_PRIMALITY) && i < MAX_NUM_TRIES);
	if (i === MAX_NUM_TRIES) throw new Error(`generatePrime reached MAX_NUM_TRIES=${MAX_NUM_TRIES}`);
	return prime;
}
function generateSafePrime(bitLength, NUM_TRIES_PRIMALITY = 20) {
	...
	let i = 0;
	do {
		prime = generatePrime(bitLength - 1, NUM_TRIES_PRIMALITY).doubleM().addM(ONE).normalize();
		i++;
	} while (!millerRabinTest(prime, NUM_TRIES_PRIMALITY) && i < MAX_NUM_TRIES);
	if (i === MAX_NUM_TRIES) throw new Error(`generateSafePrime reached MAX_NUM_TRIES=${MAX_NUM_TRIES}`);
	return prime;
}
//#endregion
//#region ../../node_modules/@cloudflare/blindrsa-ts/lib/src/partially_blindrsa.js
var PartiallyBlindRSA = class PartiallyBlindRSA {
	params;
	static NAME = "RSA-PSS";
	constructor(params) {
		this.params = params;
	...
	toString() {
		return `RSAPBSSA-${this.params.hash.replace("-", "")}-${"PSS" + (this.params.saltLength === 0 ? "ZERO" : "")}-${PrepareType[this.params.prepareType]}`;
	}
	prepare(msg) {
		const msg_prefix_len = this.params.prepareType;
		return joinAll([crypto.getRandomValues(new Uint8Array(msg_prefix_len)), msg]);
	...
	async extractKeyParams(key, type) {
		if (key.type !== type || key.algorithm.name !== PartiallyBlindRSA.NAME) throw new Error(`key is not ${PartiallyBlindRSA.NAME}`);
		if (!key.extractable) throw new Error("key is not extractable");
		const { modulusLength: modulusLengthBits, hash: hashFn } = key.algorithm;
	...
	async blind(publicKey, msg, info) {
		...
		const m = os2ip(await emsa_pss_encode(msg_prime, modulusLength - 1, opts));
		if (!is_coprime(m, n)) throw new Error("invalid input");
		const r = random_integer_uniform(n, kLen);
		let inv;
		try {
		...
		const x = rsavp1(await this.derivePublicKey(pk, info), r);
		return {
			blindedMsg: i2osp(m.mulmod(x, n), kLen),
			inv
		};
	...
	async blindSign(privateKey, blindMsg, info) {
		const { jwkKey, modulusLengthBytes: kLen } = await this.extractKeyParams(privateKey, "private");
		if (!jwkKey.n || !jwkKey.d || !jwkKey.p || !jwkKey.q) throw new Error("key has invalid parameters");
		const sk = {
			n: sjcl.bn.fromBits(sjcl.codec.base64url.toBits(jwkKey.n)),
			d: sjcl.bn.fromBits(sjcl.codec.base64url.toBits(jwkKey.d)),
			p: sjcl.bn.fromBits(sjcl.codec.base64url.toBits(jwkKey.p)),
			q: sjcl.bn.fromBits(sjcl.codec.base64url.toBits(jwkKey.q))
		};
		const m = os2ip(blindMsg);
		...
		if (this.params.supportsRSARAW) {
			const { privateKey } = await PartiallyBlindRSA.bigKeyPairToCryptoKeyPair({
				secretKey: sk_derived,
				publicKey: pk_derived
			}, {
	...
	async finalize(publicKey, msg, info, blindSig, inv) {
		const { jwkKey, modulusLengthBytes: kLen } = await this.extractKeyParams(publicKey, "public");
		if (!jwkKey.e || !jwkKey.n) throw new Error("key has invalid parameters");
		const e = sjcl.bn.fromBits(sjcl.codec.base64url.toBits(jwkKey.e));
		const n = sjcl.bn.fromBits(sjcl.codec.base64url.toBits(jwkKey.n));
		const pk = {
			e,
		...
		if (!await crypto.subtle.verify(algorithm, pk_derived_key, sig.slice().buffer, msg_prime)) throw new Error("invalid signature");
		return sig;
	}
	static async generateKey(algorithm, generateSafePrimeSync = generateSafePrime) {
		...
		const e = new sjcl.bn("0x" + Array.from(algorithm.publicExponent).map((x) => x.toString(16).padStart(2, "0")).join(""));
	...
	async verify(publicKey, signature, message, info) {
		...
		return crypto.subtle.verify({
			name: PartiallyBlindRSA.NAME,
			saltLength: this.params.saltLength
		}, pk_derived_key, signature.slice().buffer, msg_prime);
	}
	async derivePublicKey({ n }, info) {
		const hkdf_input = joinAll([
		...
		return {
			e: os2ip(expanded_bytes.slice(0, lambda_len)),
			n
		};
	}
...
//#endregion
//#region ../../node_modules/@cloudflare/blindrsa-ts/lib/src/index.js
var Params = {
...
var RSAPBSSA = { SHA384: {
	generateKey: (algorithm) => PartiallyBlindRSA.generateKey({
		...algorithm,
		hash: "SHA-384"
	}),
	PSS: {
		Randomized: (params = { supportsRSARAW: false }) => new PartiallyBlindRSA({
			...Params.RSAPBSSA_SHA384_PSS_Randomized,
			...params
		}),
		Deterministic: (params = { supportsRSARAW: false }) => new PartiallyBlindRSA({
			...Params.RSAPBSSA_SHA384_PSS_Deterministic,
			...params
		})
	},
	PSSZero: {
		Randomized: (params = { supportsRSARAW: false }) => new PartiallyBlindRSA({
			...Params.RSAPBSSA_SHA384_PSSZERO_Randomized,
			...params
		}),
		Deterministic: (params = { supportsRSARAW: false }) => new PartiallyBlindRSA({
...
function getSuiteByName(newT, name, params = { supportsRSARAW: false }) {
	for (const suiteParams of Object.values(Params)) if (name.toLowerCase() === suiteParams.name.toLowerCase()) return new newT({
		...suiteParams,
		...params
	});
...
//#endregion
export { BlindRSA, Params, PartiallyBlindRSA, RSABSSA, RSAPBSSA, getSuiteByName };

//# sourceMappingURL=@cloudflare_blindrsa-ts.js.map