...
import * as m$7 from "stream";
import * as m$6 from "http";
import * as m$5 from "url";
import * as m$4 from "https";
import * as m$3 from "zlib";
import * as m$2 from "util";
import * as m from "fs";
...
//#endregion
//#region ../../node_modules/node-fetch/lib/index.es.js
...
function Body(body) {
	...
	else if (typeof body === "string") {} else if (isURLSearchParams(body)) {} else if (body instanceof Blob$1) {} else if (Buffer.isBuffer(body)) {} else if (Object.prototype.toString.call(body) === "[object ArrayBuffer]") {} else if (body instanceof Stream) {} else body = String(body);
	this[INTERNALS] = {
		body,
		disturbed: false,
		error: null
...
function consumeBody() {
	...
	if (this.body === null) return Body.Promise.resolve(Buffer.alloc(0));
	if (typeof this.body === "string") return Body.Promise.resolve(Buffer.from(this.body));
	if (this.body instanceof Blob$1) return Body.Promise.resolve(this.body[BUFFER]);
	if (Buffer.isBuffer(this.body)) return Body.Promise.resolve(this.body);
	if (Object.prototype.toString.call(this.body) === "[object ArrayBuffer]") return Body.Promise.resolve(Buffer.from(this.body));
...
function extractContentType(instance) {
	...
	else if (body instanceof Blob$1) return body.type || null;
...
function getTotalBytes(instance) {
	...
	else if (typeof body === "string") return Buffer.byteLength(body);
	else if (isURLSearchParams(body)) return Buffer.byteLength(String(body));
	else if (body instanceof Blob$1) return body.size;
	else if (Buffer.isBuffer(body)) return body.length;
	else if (Object.prototype.toString.call(body) === "[object ArrayBuffer]") return body.byteLength;
...
function validateName(name) {
	name = `${name}`;
	if (invalidTokenRegex.test(name)) throw new TypeError(`${name} is not a legal HTTP header name`);
}
...
function getNodeRequestOptions(request) {
	...
	if (request.compress) headers.set("Accept-Encoding", "gzip,deflate");
	if (!headers.has("Connection") && !request.agent) headers.set("Connection", "close");
...
var BUFFER, TYPE, Blob$1, Stream, PassThrough, convert, INTERNALS, invalidTokenRegex, invalidHeaderCharRegex, MAP, Headers, INTERNAL, HeadersIteratorPrototype, STATUS_CODES, INTERNALS$1, Response, _require$2, format_url, parse_url, INTERNALS$2, Request, http, https, PassThrough$1, resolve_url, zlib;
var init_index_es = __esmMin(() => {
	...
	Stream = require_builtin_esm_external_require_stream();
	PassThrough = require_builtin_esm_external_require_stream().PassThrough;
	try {
 ...
 // istanbul ignore else: future proof
	...
	STATUS_CODES = require_builtin_esm_external_require_http().STATUS_CODES;
	...
	_require$2 = require_builtin_esm_external_require_url();
	format_url = _require$2.format;
	parse_url = _require$2.parse;
	...
	Request = class Request {
		constructor(input) {
			...
			let parsedURL;
			if (!isRequest(input)) {
				if (input && input.href) parsedURL = parse_url(input.href);
				else parsedURL = parse_url(`${input}`);
				input = {};
			} else parsedURL = parse_url(input.url);
			...
			if (init.body != null) {
				...
				if (contentType !== null && !headers.has("Content-Type")) headers.append("Content-Type", contentType);
			}
			this[INTERNALS$2] = {
				method,
	...
	http = require_builtin_esm_external_require_http();
	https = require_builtin_esm_external_require_https();
	PassThrough$1 = require_builtin_esm_external_require_stream().PassThrough;
	resolve_url = require_builtin_esm_external_require_url().resolve;
	zlib = require_builtin_esm_external_require_zlib();
	/**
...
var require_builtin_esm_external_require_util = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
var nr = er((function(t) {
	...
	(function(e, n) {
		...
		if (n["seed" + s] = f, v(n.random(), e), t.exports) {
			t.exports = f;
			try {
				r = require_builtin_esm_external_require_crypto();
			} catch (t) {}
		}
	})([], Math);
}));
...
var rf, of = function() {
	return init_index_es(), __toCommonJS(index_es_exports);
}, af = function() {
	function t() {
		this.util = require_builtin_esm_external_require_util(), this.textEncoder = new this.util.TextEncoder();
	}
	return t.prototype.fetch = function(t, e) {
...
var require_builtin_esm_external_require_fs = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
function createFileSystem(fs) {
	var requireFsError = "";
	if (!fs) try {
		fs = require_builtin_esm_external_require_fs();
	} catch (err) {
