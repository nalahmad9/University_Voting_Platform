...
function phaseDetails(ballot: StudentBallotRecord): {
  ...
  return {
    label: "Closed",
    tone: "bg-[#ece9e5] text-[#5e5752]",
    dateLabel: `Voting closed ${new Date(ballot.endTime).toLocaleString()}`,
    action: "View results",
...
export function StudentBallotDashboard({
  ...
  return <>
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
      <div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#8f2f43]">Student dashboard</p><h1 className="mt-2 text-4xl font-bold sm:text-5xl">Good morning, {user.fullName.split(" ")[0]}.</h1><p className="mt-2 max-w-3xl text-[#6e665f]">Your eligible ballots, current deadlines and next actions are gathered here.</p></div>
    ...
    {latestUnread&&<section role="status" className="mt-8 rounded-2xl border border-[#e3ca92] bg-[#fff6d9] p-4 sm:flex sm:items-center sm:justify-between">
