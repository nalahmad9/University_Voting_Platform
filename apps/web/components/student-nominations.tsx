...
export function StudentNominations({
  ...
  return <>
    ...
    {opportunities.length>1&&<label className="mt-7 block max-w-xl font-semibold">Choose a ballot<select value={selectedBallotId} onChange={event=>setSelectedBallotId(event.target.value)} className={fieldClass}>{opportunities.map(item=><option key={item.ballot.id} value={item.ballot.id}>{item.ballot.title}</option>)}</select></label>}
